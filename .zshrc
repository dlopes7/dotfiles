...
c () {
    ...
    if [[ $# -gt 1 ]]; then
        echo "Usage: c [filter]"
        echo "  c          - list all directories"
        echo "  c <filter> - list directories matching filter"
        return
    fi
    
    # Set the search pattern based on whether a filter is provided
    matches=()
    if [[ $# -eq 1 ]]; then
        for dir in ~/workspace/{DPE2,DEXT2}/*"$1"*; do
            if [[ -d $dir ]]; then
                matches+=($dir)
            fi
        done
    else
        for dir in ~/workspace/{DPE2,DEXT2}/*; do
            if [[ -d $dir ]]; then
                matches+=($dir)
            fi
        done
    ...
    for i in {1..${#matches}}; do
        echo "$i) ${matches[$i]:h:t}/${matches[$i]:t}"
    done

    read "selection?Enter number: "
    if [[ $selection =~ ^[0-9]+$ ]] && (( selection <= ${#matches} && selection > 0 )); then
